...
_SI4710_CMDsetProperty = const(0x12)
...
class SI4713:
    ...
    def setProperty(self, prop: int, val: int) -> None:
        # Set a property of the SI4713 chip.  These are both 16-bit values.
        self._BUFFER[0] = _SI4710_CMDsetProperty
        self._BUFFER[1] = 0
    ...
    def reset(self) -> None:
        ...
        # crystal is 32.768
        self.setProperty(_SI4713_PROP_REFCLK_FREQ, 32768)
        # 74uS pre-emph (USA std)
        self.setProperty(_SI4713_PROP_TX_PREEMPHASIS, 0)
        # max gain?
        self.setProperty(_SI4713_PROP_TX_ACOMP_GAIN, 10)
        # turn on limiter and AGC
        self.setProperty(_SI4713_PROP_TX_ACOMP_ENABLE, 0x0)
    ...
    def configure_rds(
        ...
        self.setProperty(_SI4713_PROP_TX_AUDIO_DEVIATION, 6625)
        # 2KHz (default)
        self.setProperty(_SI4713_PROP_TX_RDS_DEVIATION, 200)
        # RDS IRQ
        self.setProperty(_SI4713_PROP_TX_RDS_INTERRUPT_SOURCE, 0x0001)
        # program identifier
        self.setProperty(_SI4713_PROP_TX_RDS_PI, program_id)
        # 50% mix (default)
        self.setProperty(_SI4713_PROP_TX_RDS_PS_MIX, 0x03)
        # RDSD0 & RDSMS (default)
        self.setProperty(_SI4713_PROP_TX_RDS_PS_MISC, 0x1808)
        # 3 repeats (default)
        self.setProperty(_SI4713_PROP_TX_RDS_PS_REPEAT_COUNT, 3)
        self.setProperty(_SI4713_PROP_TX_RDS_MESSAGE_COUNT, 1)
        # no AF
        self.setProperty(_SI4713_PROP_TX_RDS_PS_AF, 0xE0E0)
        self.setProperty(_SI4713_PROP_TX_RDS_FIFO_SIZE, 0)
        self.setProperty(_SI4713_PROP_TX_COMPONENT_ENABLE, 0x0007)
